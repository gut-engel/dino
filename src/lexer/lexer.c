...
static Token lexer_identifier(Lexer *lexer, size_t start_line, size_t start_col) {
    ...
    if (sv_eq(lexeme, sv_from_cstr("class"))) return make_token(TOKEN_CLASS, lexeme, start_line, start_col);
    ...
    if (sv_eq(lexeme, sv_from_cstr("delete"))) return make_token(TOKEN_DELETE, lexeme, start_line, start_col);
